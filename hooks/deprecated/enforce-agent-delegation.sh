...
- Context search → ctx "query" (auto-saves to \$ctx_last)
- LLM dispatch → llm codex "task @var:ctx_last"
...
**Variable Protocol**: Always use \`ctx\` → \`llm @var:ctx_last\` for 99% token savings
